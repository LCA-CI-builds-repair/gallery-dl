...
Type
    * ``string``
    * ``object``    ``pawoo`` and ``baraag``. For other instances, you need to obtain an
    ``access-token`` in order to use usernames in place of numerical
    user IDs.ndition` -> `format string`_)
Example
    .. code:: json
...
Example
    ...
    .. code:: json

        {
            ...
            "retwee    with a display width greater than 1.d != 0"    : ["{category}", "{user[name]}", "Retweets"],
            ""                   : ["{category}", "{user[name]}"]
        }
